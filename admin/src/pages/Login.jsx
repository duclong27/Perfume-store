...
export default function AdminLoginForm({ setToken }) {
    ...
    async function onSubmit(e) {
        ...
        const endpoint = "/api/user/loginAdminOrStaff";
        console.log("[DEMO LOGIN] POST", (api.defaults.baseURL || "") + endpoint);

        try {
            // ✅ DEMO MODE: KHÔNG gửi body
            ...
            // Demo: dùng sessionStorage là đủ
            sessionStorage.setItem("token", data.token);
            localStorage.removeItem("token");
            ...
            <div className="relative w-full max-w-md">
                <div className="rounded-3xl bg-white/10 backdrop-blur-xl
                    ...
                    <p className="text-white/80 text-center mb-8">
                        Click below to explore the admin system demo.
                    </p>
                    ...
                    <form onSubmit={onSubmit} className="space-y-6">
                        <button
                            type="submit"
                            disabled={loading}
                            className="w-full rounded-xl bg-[#7A58FF]
                                       hover:bg-[#6B4BFF]
                                       transition-colors py-3 font-medium
                                       shadow-lg shadow-black/10
                                       disabled:opacity-70"
                        >
                            {loading ? "Entering demo…" : "Try Demo"}
                        </button>

                        {error && (
                            <p
                                role="alert"
                                className="text-pink-200 text-sm text-center"
                            >
                                {error}
                            </p>
                        )}
                ...
                {/* Glow */}
                <div className="absolute -inset-2 -z-10 rounded-[2rem]
                                bg-gradient-to-br from-white/10
