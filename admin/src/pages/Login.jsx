...
// src/pages/AdminLoginForm.jsx
import { useState } from "react";
import { useLocation, useNavigate } from "react-router-dom";
import axios from "axios";
import { api } from "../App";

/** ✅ FIX: helper lưu token đúng storage theo remember
 *  - Tác dụng: App.jsx (getStoredToken) sẽ đọc được token,
 *    không còn tình trạng “login xong vẫn bị đá về /login”.
 */
...
export default function AdminLoginForm({ setToken }) {
    ...
    async function onSubmit(e) {
        ...
        try {
            /** ✅ FIX: log gọn, tránh tự cộng chuỗi baseURL dễ sai dấu /
             *  - Tác dụng: debug rõ baseURL và endpoint đang gọi.
             */
            console.log("[API] baseURL =", api.defaults.baseURL); // ✅ FIX

            const endpoint = "/api/user/demoAdminLogin"; // ✅ FIX: đổi nếu backend khác
            console.log("[LOGIN] endpoint =", endpoint); // ✅ FIX
            ...
            /** ✅ FIX: hỗ trợ backend trả token dưới tên khác
             *  - Tác dụng: tránh “không nhận được token” do BE trả accessToken.
             */
            const t = data?.token || data?.accessToken; // ✅ FIX
            if (!t) {
                throw new Error("Không nhận được token từ server");
            }

            setStoredToken(t, remember); // ✅ FIX: lưu token đúng nơi
            ...
            if (axios.isAxiosError(err)) {
                if (!err.response) {
                    setError(
                        "Network error: không thể kết nối server. Kiểm tra API URL/CORS/backend."
                    );
                    console.error("[AXIOS NETWORK ERROR]", err.message, err.code, err.toJSON?.());
                } else {
                    const msg = err.response.data?.message || `HTTP ${err.response.status}`;
            ...
            <div className="relative w-full max-w-md">
                <div className="rounded-3xl max-w-md bg-white/10 backdrop-blur-xl shadow-2xl border border-white/10 p-8 text-white">
                    ...
                    <form onSubmit={onSubmit} className="space-y-4">
                        ...
                        {error && (
                            <p role="alert" className="text-pink-200 text-sm text-center">
                                {error}
                            </p>
                        )}
                    </form>
