...
const methodTone = (m) => {
    switch (m) {
        case "COD": return "bg-indigo-500/20 text-indigo-300 border-indigo-500/30";
        case "BANK_TRANSFER": return "bg-fuchsia-500/20 text-fuchsia-300 border-fuchsia-500/30";
        case "VNPAY": return "bg-cyan-500/20 text-cyan-300 border-cyan-500/30";
...
const payTone = (s) => {
    switch (s) {
        ...
        case "pending": return "bg-amber-500/20 text-amber-300 border-amber-500/30";
        case "unpaid": return "bg-slate-500/20 text-slate-300 border-slate-500/30";
        case "cancelled": return "bg-rose-500/20 text-rose-300 border-rose-500/30";
        default: return "bg-slate-500/20 text-slate-300 border-slate-500/30";
    }
};
...
function allowedPaymentTargets(cur, method) {
    ...
    if (cur === "pending") { res.add("paid"); res.add("failed"); }
    if (cur !== "paid") { res.add("failed"); }
    if (method === "VNPAY") { res.delete("paid"); }
    return Array.from(res);
}
...
export default function AdminOrdersAll() {
    ...
    async function handleSaveRow(o) {
        ...
        try {
            ...
            toast.success(`Đã cập nhật đơn #${o.orderNumber}`);
        } catch (e) {
            setOrders((prev) =>
                prev.map((x) => (x.orderId === o.orderId ? { ...oldRow, __pending: false } : x))
            );
            toast.error(e?.response?.data?.message || e?.message || "Failed to update order");
        } finally {
            setPending((p) => ({ ...p, [o.orderId]: false }));
        }
    }
    ...
    return (
        <div className="rounded-3xl w-full min-h-screen border border-white/15 bg-white/5 backdrop-blur-lg p-[clamp(16px,2vw,32px)] text-white mt-6">
            ...
            <div className="flex flex-col md:flex-row items-start md:items-center justify-between mb-[clamp(20px,2vw,28px)] gap-4">
                <div>
                    <h2 className="text-[clamp(24px,2.6vw,32px)] font-extrabold tracking-tight">Orders Management</h2>
                    <p className="text-white/70 text-[clamp(13px,1.6vw,16px)] mt-1">
                        {orders.length} orders loaded • {sorted.length} showing
                    </p>
                </div>

                <div className="flex items-center gap-3 w-full md:w-auto">
                    {/* Search */}
                    <div className="flex md:flex items-center gap-2 rounded-xl bg-white/10 px-[clamp(12px,1.6vw,16px)] py-[clamp(8px,1.2vw,12px)] flex-1 md:flex-initial">
            ...
            {/* Filters */}
            <div className="mb-[clamp(16px,2vw,24px)] grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-[clamp(12px,1.6vw,16px)]">
                <select
                    value={orderStatus}
                ...
                >
                    ...
                    <option value="totalVnd">Sort: Total</option>
                    <option value="orderStatus">Sort: Order Status</option>
            ...
            <div className="w-full overflow-hidden rounded-2xl border border-white/10">
                {/* Header */}
                <div className="hidden lg:grid grid-cols-9 gap-0 bg-white/5 text-slate-300 text-[clamp(12px,1.6vw,16px)] font-semibold px-[clamp(12px,2vw,24px)] py-[clamp(12px,1.6vw,16px)]">
                    <div>Order #</div>
                    <div>Customer</div>
                    <div>Total</div>
                    <div>Method</div>
                    ...
                    {sorted.length === 0 ? (
                        ...
                        sorted.map((o) => {
                            ...
                            const orderOptions = ["pending", "confirmed", "shipped", "completed", "cancelled"]
                                ...
                                .map((opt) => {
                                    const same = (draft.orderStatus ?? o.orderStatus) === opt;
                                    const enable = same || allowFul.includes(opt) || opt === o.orderStatus;
                                    return { value: opt, label: opt, disabled: !enable };
                                });
                            ...
                            const hasChanges =
                                (draft.orderStatus !== o.orderStatus) ||
                                (draft.paymentStatus !== o.paymentStatus);

                            const isValid = validateDraft(o, draft);

                            return (
                                <div
                                    key={o.orderId}
                                    className={`grid grid-cols-1 lg:grid-cols-9 gap-[clamp(8px,1.2vw,12px)] px-[clamp(12px,2vw,24px)] py-[clamp(12px,1.6vw,16px)] hover:bg-white/5 ${isRowPending(o.orderId) ? "opacity-60 pointer-events-none" : ""
                                        }`}
                                >
                                    {/* Order # */}
                                    <div className="rounded-lg border border-white/15 bg-white/10 px-[clamp(10px,1.4vw,14px)] py-[clamp(8px,1.2vw,12px)]">
                                        <div className="font-semibold text-white truncate text-[clamp(13px,1.6vw,16px)]" title={o.orderNumber}>
                                            {o.orderNumber}
                                        </div>
                                        <div className="text-white/60 text-[clamp(11px,1.4vw,14px)] truncate">ID: {o.orderId}</div>
                                    </div>

                                    {/* Customer */}
                                    <div className="rounded-lg border border-white/15 bg-white/10 px-[clamp(10px,1.4vw,14px)] py-[clamp(8px,1.2vw,12px)]">
                                        <div className="text-white truncate text-[clamp(13px,1.6vw,16px)]" title={o.shippingName}>
                                            {o.shippingName || "—"}
                                        </div>
                                        <div className="text-white/60 text-[clamp(11px,1.4vw,14px)] truncate">{o.shippingPhone || "—"}</div>
                                    </div>

                                    {/* Total */}
                                    <div className="rounded-lg border border-white/15 bg-white/10 px-[clamp(10px,1.4vw,14px)] py-[clamp(8px,1.2vw,12px)] text-white truncate text-[clamp(13px,1.6vw,16px)]">
                                        {o.totalVnd.toLocaleString("vi-VN")} ₫
                                    </div>

                                    {/* Method */}
                                    <div className={`rounded-lg border px-[clamp(10px,1.4vw,14px)] py-[clamp(6px,1vw,8px)] text-center text-[clamp(11px,1.4vw,13px)] font-semibold truncate ${methodTone(o.paymentMethodCode)}`}>
                                        {o.paymentMethodCode || "—"}
                                    </div>

                                    {/* Order Status */}
                                    <div className="lg:col-span-2 flex flex-col sm:flex-row items-stretch sm:items-center gap-[clamp(8px,1.2vw,12px)]">
                                        <div className={`rounded-lg border px-[clamp(10px,1.4vw,14px)] py-[clamp(6px,1vw,8px)] text-[clamp(11px,1.4vw,13px)] font-semibold text-center ${statusTone(o.orderStatus)}`}>
                                            {o.orderStatus || "—"}
                                        </div>
                                        <select
                                            value={draft.orderStatus ?? o.orderStatus ?? ""}
                                        ...
                                        >
                                            {orderOptions.map((opt) => (
                                                <option key={opt.value} value={opt.value} disabled={opt.disabled}>
                                                    {opt.label}{opt.disabled && opt.value !== o.orderStatus ? " 🔒" : ""}
                                                </option>
                                            ))}
                                        </select>
                                    </div>
                                    ...
                                    {/* Payment Status */}
                                    <div className="lg:col-span-2 flex flex-col sm:flex-row items-stretch sm:items-center gap-[clamp(8px,1.2vw,12px)]">
                                        <div className={`rounded-lg border px-[clamp(10px,1.4vw,14px)] py-[clamp(6px,1vw,8px)] text-[clamp(11px,1.4vw,13px)] font-semibold text-center ${payTone(o.paymentStatus)}`}>
                                            {o.paymentStatus || "—"}
                                        </div>
                                        <select
                                            value={draft.paymentStatus ?? o.paymentStatus ?? ""}
                                        ...
                                        >
                                            {payOptions.map((opt) => (
                                                <option key={opt.value} value={opt.value} disabled={opt.disabled}>
                                                    {opt.label}{opt.disabled && opt.value !== o.paymentStatus ? " 🔒" : ""}
                                                </option>
                                            ))}
                                        </select>
                                    </div>
                                    ...
                                    <div className="flex justify-center">
                                        <button
                                            onClick={() => handleSaveRow(o)}
                                            disabled={!hasChanges || !isValid || isRowPending(o.orderId)}
